...
const RichEditor: React.FC<RichEditorProps> = ({ content, onChange }) => {
  ...
  /* FIX: Use onMouseDown with preventDefault to avoid losing editor focus */
  const ToolbarButton = ({ onClick, isActive, icon: Icon, disabled = false }: any) => (
    <button
      onMouseDown={(e) => {
        e.preventDefault(); // SUPER IMPORTANT: Prevents editor blur
        onClick(e);
      }}
      disabled={disabled}
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
      {/* TOOLBAR: Removed overflow-x-auto to allow color palette dropdown to show. Wrapped in flex-wrap. */}
      <div className="flex flex-wrap items-center gap-1 p-2 border-b border-white/10 bg-white/5 backdrop-blur-xl shrink-0 sticky top-0 z-10 transition-all">
        ...
        <div className="flex items-center gap-0.5">
          <div className="relative group">
            <button className={`p-2 rounded text-gray-400 hover:text-white hover:bg-white/10`} type="button">
              <Palette size={18} />
            </button>
            <div className="absolute top-full right-0 mt-2 bg-black/90 border border-white/20 p-2 rounded flex gap-1 z-[60] w-max invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all">
